// Gamification + temporal data (mock, hackathon-ready)
...
const ECO_USER = {
  ...
  weeklyPoints: 285,
  totalPoints: 1820,
  rankWeekly: 14,         // position out of city
  rankAllTime: 47,
  streak: 6,              // consecutive days
  pointsToNext: 215,      // points to next level
  thisWeekBreakdown: [
    { label: 'He reciclado',   count: 18, each: 10, color: '#2E8B57' },
    { label: 'Reportes válidos', count: 5, each: 15, color: '#005A9C' },
    { label: 'Banner satisfacción', count: 6, each: 5, color: '#E8A317' },
  ],
};

// Top 100 (we'll only render top ~20 + the user's nearby slice + bottom hint)
const ECO_LEADERBOARD = [
  ...
  { rank: 11, name: 'Carmen Hdez. Toledo',  barrio: 'Centro',     pts: 384, delta: 2,  initials: 'CH', avatar: '#A4243B' },
...
// Fullness probability by hour of day, per container type (0..1)
const ECO_HOURLY = {
...
// Heuristic for "best window" — returns first window where prob < 0.4
function bestHourTip(containerType) {
  const arr = ECO_HOURLY[containerType] || ECO_HOURLY.resto;
  // peak hour
  let peakH = 0, peakV = 0;
  arr.forEach((v, h) => { if (v > peakV) { peakV = v; peakH = h; } });
  // best window after peak
  let bestStart = -1;
  ...
  return {
    peakHour: peakH,
    bestHour: bestStart,
    peakPct: Math.round(peakV * 100),
  };
}

// Demo "route" — picked containers in order
const ECO_ROUTE_DEMO = [
  { reportId: 'C-501', container: 'envases', addr: 'Calle Castillo, 47', area: 'Centro',  x: 0.42, y: 0.48, distance: '120 m', eta: '2 min' },
  { reportId: 'C-714', container: 'vidrio',  addr: 'Rambla 25 Julio, 8', area: 'Rambla',  x: 0.28, y: 0.36, distance: '380 m', eta: '5 min' },
  { reportId: 'C-298', container: 'papel',   addr: 'Plaza del Príncipe', area: 'Centro',  x: 0.36, y: 0.58, distance: '620 m', eta: '8 min' },
];

Object.assign(window, {
  ECO_USER, ECO_LEADERBOARD, ECO_HOURLY, ECO_ROUTE_DEMO, bestHourTip,
});
